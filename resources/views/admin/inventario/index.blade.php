...
@section('title', 'Informacion de inventario')
@section('contenido')
                                        ...
                                        <div class="dataTables_length" id="order-listing_length">
                                                <form class="form-inline">
                                                    <div>
                                                        ...
                                                        <a href="{{ route('inventarios.create') }}" class="btn btn-dark ">
                                                            + Agregar nuevo
                                                        </a>
                                                        @endcan
                                                    </div>
                                              ...
                                              @foreach ($inventarios as $inventario)
                                                <tr>
                                                    ...
                                                    <td>{{ $inventario->Stock }}</td>  
                                                     ...
                                                     <td style="width: 20%;"> 
                                                         {{--  {!! Form::open(['route' => ['inventarios.destroy', $inventario], 'method' => 'DELETE']) !!}    --}}
                                                        
                                                         @can('product_show')
                                                        <a class="btn btn-outline-warning"
                                                            href="{{ route('inventarios.show', $inventario) }}">
                                                            <i class="fa fa-eye" aria-hidden="true"></i></a>
                                                            @endcan
                                                            @can('product_edit')
                                                            <a class="btn btn-outline-info"
                                                            href="{{ route('inventarios.edit', $inventario) }}" title="Editar">
                                                            <i class="far fa-edit"></i>
                                                        </a>
                                                        @endcan
                                                        @can('product_destroy')
                                                        {{--  <a href="#" class="btn btn-outline-danger" data-toggle="modal" data-target="#modal-delete-user{{$inventario->id}}"><i class="far fa-trash-alt"></i></a>    --}}
                                                        
                                                    </td>
                                                    {{--  @include('admin.inventario.modal.delete')    --}}
                                                   {{--  {!! Form::close() !!}    --}}
                                                      @endcan
                                                </tr>
                                            @endforeach
