  ...
  {!! Form::model($client, ['route' => ['clients.update', $client], 'method' => 'PUT']) !!}
            <input style="display:none" type="text" name="name" id="name" value="{{ $client->name }}"
            ...
            <input  style="display:none" type="number" name="ci" id="ci" value="{{ $client->ci }}"
            class="form-control" placeholder="" required 
            ...
            <input style="display:none" type="email" name="email" id="email" value="{{ $client->email }}"
        class="form-control" placeholder="" required>
            <input  style="display:none" type="number" name="phone" id="phone" value="{{ $client->phone }}"
            class="form-control" placeholder="" required
            oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" ng-model="number" 
            onKeyPress="if(this.value.length==8) return false;" min="0" />
            <select style="display:none" name="estatus" value="{{ old('estatus', $client->estatus) }}" class="form-control" id="exampleSelectGender">
            <option value="0">Inactivo</option>
            </select>                                                                                                                       
  <div class="modal fade m-medium" id="modal-delete-edit{{$client->id}}" tabindex="-1" role="dialog" aria-labelledby="modal-delete-edit">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-body">
          ...
          <center> <h4>El registro N:<b>{{ $client->id}}</b></h4></center>
